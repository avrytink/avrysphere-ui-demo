...
export const useOSStore = create<OSState>((set, get) => ({
  ...
  setTheme: (theme) => {
    ...
    useAuthStore.getState().updateUser({
      settings: {
        ...useAuthStore.getState().currentUser?.settings,
        theme,
      } as any,
    });
  },
  ...
  setBatteryStatus: (level, charging) =>
    set({ batteryLevel: level, isCharging: charging }),
  ...
  launchUrl: (url: string) => {
    ...
    switch (target.toLowerCase()) {
      case "settings":
        windowStore.openWindow(AppId.SETTINGS, false, {
          tab: subPath.toUpperCase(),
          ...args,
        });
        break;
      case "music":
        ...
        break;
      case "ai":
        dockStore.setAIPanelOpen(true);
        break;
      case "browser":
        windowStore.openWindow(AppId.BROWSER, false, {
          url: args.url || "https://www.bing.com",
        });
        break;
      case "store":
      case "shopper":
