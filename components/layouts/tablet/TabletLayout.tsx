...
import { LauncherPanel } from "../../momentDock/panels/LauncherPanel";
import { MobileNavBar } from "../mobile/MobileNavBar";

export const TabletLayout: React.FC = () => {
  const { brightness, switcherOpen, closeEverything } = useOSStore();
  const { currentUser } = useAuthStore();
  const { windows, activeWorkspaceId, workspaces, blurWindows, minimizeAll } =
    useWindowStore();
  ...
  return (
    ...
    >
      ...
      */}
      <main
        className={`fixed inset-0 top-0 overflow-hidden transition-all duration-700 ${
          showHomeScreen ? "bottom-32" : "bottom-0"
        }`}
        onClick={handleDesktopClick}
        style={{
