...
export const DesktopLayout: React.FC = () => {
  ...
  const { windows, activeWorkspaceId, workspaces, blurWindows } =
    useWindowStore();
    ...
    if (!appDef) {
      return (
        <div className="p-20 text-center flex flex-col items-center justify-center h-full gap-4">
          <div className="text-red-600 text-4xl font-black uppercase tracking-widest opacity-20">
            System Error
          </div>
          <div className="text-[10px] text-zinc-500 uppercase tracking-[1em]">
            Missing Modality: {appId}
          </div>
        </div>
      );
    }
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-black relative animate-in fade-in duration-1000">
      ...
      <main
        className={`fixed right-0 overflow-hidden transition-all duration-700 ${
          isUnity ? "top-0" : "top-8"
        } bottom-0 ${
          isUnity ? (dockCollapsed ? "left-12" : "left-20") : "left-0"
        }`}
        onClick={handleDesktopClick}
