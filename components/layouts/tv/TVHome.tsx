...
const FEATURED_CONTENT = [
  ...
  },
  {
    ...
    image:
      "https://images.unsplash.com/photo-1541963463532-d68292c34b19?q=80&w=1976",
    type: "Movie",
  },
  {
    id: "3",
...
export const TVHome: React.FC = () => {
  ...
  const apps = Object.values(APP_REGISTRY).filter(
    (app) =>
      (!app.headless ||
        [AppId.DESKTOP_MODE, AppId.TABLET_MODE, AppId.GAME_MODE].includes(
          app.id
        )) &&
          ...
          {FEATURED_CONTENT.map((_, i) => (
            <div
              ...
              className={`w-3 h-3 rounded-full transition-all duration-300 ${
                i === heroIndex ? "bg-white scale-125" : "bg-white/30"
              }`}
        ...
        <div className="flex gap-6 overflow-x-auto px-4 pb-8 no-scrollbar scroll-smooth">
          {apps.map((app) => (
            ...
            >
              ...
              <span className="text-sm font-medium text-zinc-400 group-hover:text-white transition-colors text-center w-36 truncate">
                {app.title}
              </span>
            </button>
          ))}
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-8 overflow-x-auto px-4 pb-4 no-scrollbar">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="flex-shrink-0 w-80 aspect-video rounded-2xl bg-zinc-800 overflow-hidden relative group cursor-pointer border border-white/5 hover:border-white/40 transition-all hover:scale-105"
            >
              <img
                src={`https://images.unsplash.com/photo-1614850523296-d8c1af93d400?auto=format&fit=crop&q=80&w=600&text=${i}`}
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-6 overflow-x-auto px-4 pb-4 no-scrollbar">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="flex-shrink-0 w-48 aspect-[2/3] rounded-2xl bg-zinc-800 overflow-hidden relative group cursor-pointer border border-white/5 hover:border-white/40 transition-all hover:scale-105"
            >
              <img
                src={`https://images.unsplash.com/photo-1635776062127-d379bfcba9f8?auto=format&fit=crop&q=80&w=400&text=${i}`}
                className="w-full h-full object-cover"
                alt=""
              />
            </div>
