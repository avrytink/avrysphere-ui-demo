...
import { ChevronRight, Lock, Power, Mail, Loader2, Sparkles, Plus, ShieldCheck, Building2 } from 'lucide-react';
...
const useDragScroll = () => {
  ...
  return { ref, events: { onMouseDown, onMouseLeave, onMouseUp, onMouseMove }, onClick };
};

export const AuthScreen: React.FC = () => {
  const { mockUsers, getUserOrganizations } = useUserStore();
  const { isAuthenticated, isLocked, unlockSession, currentUser, loggedInUsers, powerDown, login } = useAuthStore();
  ...
  const { 
    ...
    isEntering, completeEntrance,
    isSendingLink, linkSent, requestMagicLink,
    error, triggerError
  } = useAuthUIStore();
  ...
  useEffect(() => {
    ...
    if (!selectedUser && !isAddingAccount) {
      if (isAuthenticated && currentUser) {
        ...
        setSelectedOrgId(lastUser.lastActiveOrgId || lastUser.organizationIds[0]);
      } else {
  ...
  }, [isAuthenticated, currentUser, loggedInUsers, mockUsers, selectedUser, setSelectedUser, setIsAddingAccount, completeEntrance, isAddingAccount, setSelectedOrgId]);
  ...
  useEffect(() => {
    if (selectedUser && userOrgs.length === 1 && !selectedOrgId && !isAddingAccount) {
      setSelectedOrgId(userOrgs[0].id);
    }
  }, [selectedUser, userOrgs, selectedOrgId, isAddingAccount, setSelectedOrgId]);

  const handleMagicLinkRequest = (e: React.FormEvent) => {
    e.preventDefault();
    if (isSendingLink) return;

    const foundUser = mockUsers.find(u => u.email.toLowerCase() === emailInput.trim().toLowerCase());
    if (!foundUser) {
       triggerError();
       return;
    }
  ...
  const backgroundUrl = selectedUser?.background || 'https://i.imgur.com/zJ4iCUp.jpeg';
  ...
  return (
    <div className="fixed inset-0 z-[5000] flex flex-col items-center justify-center bg-black overflow-hidden font-sans select-none">
      ...
      <div className={`relative z-10 w-full flex flex-col items-center transition-all duration-1000 ease-out ${isEntering ? 'opacity-0 translate-y-12' : 'opacity-100 translate-y-0'}`}>
        ...
        <div className="flex items-center gap-2 mb-8 h-6">
           <Sparkles size={12} className="text-blue-400 opacity-80" />
           <span className="text-[11px] text-zinc-300 font-medium tracking-wide animate-in fade-in slide-in-from-bottom-1 duration-700">
             {isLoginMode 
               ? "Step into your digital sanctuary." 
               : (selectedUser 
                   ? `Welcome back, ${selectedUser.name.split(' ')[0]}. Your workspace is ready.` 
                   : "Ready for your creative flow.")}
           </span>
        </div>
          ...
          <div className="w-full max-w-6xl px-8 mb-0">
            ...
            >
              {loggedInUsers.map((user) => {
                ...
                const displayName = firstName.length > 8 ? `${firstName.slice(0, 8)}...` : firstName;
                ...
                return (
                    <div 
                      ...
                      onClick={userDock.onClick(() => {
                          ...
                          setSelectedOrgId(user.lastActiveOrgId || user.organizationIds[0]);
                      })}
                      className={`relative transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] flex flex-col items-center group snap-center shrink-0 ${isSelected ? 'scale-100 z-10 opacity-100' : 'scale-90 opacity-50 hover:opacity-100'}`}
                    >
                      {/* Name - Updated Typography & Logic */}
                      <div className={`absolute -top-8 text-[12px] font-normal text-white tracking-wide whitespace-nowrap transition-all duration-500 ${isSelected ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                        {displayName}
                      </div>

                      <div className={`w-24 h-24 rounded-[2.5rem] overflow-hidden border-2 transition-all duration-500 shadow-2xl ${isSelected ? 'border-white ring-4 ring-white/10 shadow-white/10' : 'border-white/10 group-hover:border-white/30'}`}>
              ...
              >
                ...
                <span className="text-[8px] font-bold uppercase tracking-widest opacity-50 group-hover:opacity-100">Add</span>
              </button>
            </div>
          </div>
        )}
            ...
            <div className="w-full max-w-6xl px-8 mt-2 mb-8 animate-in fade-in slide-in-from-bottom-2 duration-700">
                ...
                >
                    {userOrgs.map((org) => {
                        ...
                        const bgImage = owner?.background || 'https://i.imgur.com/zJ4iCUp.jpeg';
                        ...
                        return (
                            <button
                                ...
                                className={`
                                    ...
                                    ${isActive ? 'opacity-100 scale-100' : 'opacity-50 scale-90 hover:opacity-100 hover:scale-95'}
                                `}
                            >
                                <div className={`
                                ...
                                `}>
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <Building2 size={20} className={`drop-shadow-md transition-all duration-500 ${isActive ? 'text-white scale-110' : 'text-white/70 group-hover:text-white group-hover:scale-105'}`} />
                                    </div>
                                </div>
                                ...
                                <span className={`
                                    ...
                                    ${isActive ? 'text-white translate-y-0 opacity-100' : 'text-zinc-500 -translate-y-1 opacity-0 group-hover:opacity-100 group-hover:translate-y-0'}
                                `}>
                                    {org.name}
                                </span>
          ...
          <div className="w-full relative min-h-[80px]">
            ...
            {linkSent && (
              <div className="flex flex-col items-center gap-4 animate-in zoom-in-95 duration-500">
                ...
                <span className="text-[11px] font-black text-emerald-500 uppercase tracking-[0.4em]">Synthesis Successful</span>
              </div>
            )}

            {/* Password / Email Input */}
            {(showPasswordInput || isLoginMode) && !linkSent && (
              <form onSubmit={isLoginMode ? handleMagicLinkRequest : handleUnlock} className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className={`relative group transition-all ${error ? 'animate-shake' : ''}`}>
                  <div className="relative flex items-center bg-white/[0.08] border border-white/10 rounded-[1.5rem] backdrop-blur-3xl overflow-hidden transition-all focus-within:border-white/40 focus-within:bg-white/[0.12] shadow-2xl">
                    ...
                    <input 
                      ...
                      placeholder={isLoginMode ? "Domain Email" : "Identity Passcode"}
                      value={isLoginMode ? emailInput : passwordInput}
                      onChange={(e) => isLoginMode ? setEmailInput(e.target.value) : setPasswordInput(e.target.value)}
                      className="w-full bg-transparent py-6 pl-4 pr-16 text-xs font-bold outline-none text-white placeholder:text-zinc-600"
                    />

                    <button 
                      type="submit"
                      disabled={isLoginMode ? (!emailInput.includes('@') || isSendingLink) : (!passwordInput || isSendingLink)}
                      className={`
                        absolute right-2 w-12 h-12 rounded-2xl flex items-center justify-center transition-all
                        ${((isLoginMode ? emailInput.includes('@') : passwordInput) && !isSendingLink)
                    ...
                    {isSendingLink && (
                       <div className="absolute right-4">
                          <Loader2 size={20} className="animate-spin text-white opacity-40" />
                       </div>
                    )}
                  </div>
                </div>
      ...
      <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-10">
        ...
        >
          ...
          <span className="text-[9px] font-black text-zinc-700 uppercase tracking-[0.5em] group-hover:text-zinc-400 transition-colors">Power Down</span>
        </div>
      </div>
