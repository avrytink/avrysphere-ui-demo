...
import { Wifi, BatteryCharging, Bluetooth, Bell } from "lucide-react";
...
export const TopBar: React.FC = () => {
  ...
  const activeMembers = useMemo(() => {
    ...
    if (!org) return [];
    return loggedInUsers.filter(
      (u) => org.members.includes(u.id) && u.id !== currentUser?.id
    );
  ...
  const hasMaximizedWindow = useMemo(
    () =>
      windows.some(
        (w) =>
          w.workspaceId === activeWorkspaceId && w.isMaximized && !w.isMinimized
      ),
  ...
  const Divider = () => {
    return (
      <div
        className={`w-px h-4 ${
          isDark || hasMaximizedWindow ? "bg-white/10" : "bg-white/20"
        }`}
      />
    );
  };
  ...
  const textColor =
    isDark || hasMaximizedWindow ? "text-white" : "text-zinc-900";
  const mutedTextColor =
    isDark || hasMaximizedWindow ? "text-zinc-400" : "text-zinc-600";
  const hoverBg =
    isDark || hasMaximizedWindow ? "hover:bg-white/10" : "hover:bg-black/5";
  const toggleColor =
    isDark || hasMaximizedWindow ? "bg-zinc-400" : "bg-zinc-600";
  ...
  const BatteryDisplay = () => {
    if (isCharging) {
      return (
        <div
          ...
          title={`${Math.round(batteryLevel * 100)}% Charging`}
        >
          <BatteryCharging
            size={16}
            className={
              isUnity && !isMobile && !isTablet && !hasMaximizedWindow
                ? mutedTextColor
    ...
    return (
      <div
        ...
        title={`${Math.round(levelPercent)}%`}
      >
        <div className="relative flex items-center">
          <div
            className={`w-[18px] h-[9px] border rounded-[2px] p-[1.5px] ${batBorderColor}`}
          >
            <div
              className={`h-full rounded-[0.5px] transition-all duration-500 ${fillColor}`}
              style={{ width: `${levelPercent}%` }}
            />
          </div>
          <div
            className={`w-[2px] h-[3px] -ml-[1px] rounded-r-sm ${
              isUnity && !isMobile && !isTablet && !hasMaximizedWindow
                ? isDark
                  ? "bg-zinc-500"
                  : "bg-zinc-400"
  ...
  return (
    <>
      <div
        className={`fixed top-0 left-0 right-0 z-[1000] flex items-center justify-between px-4 select-none transition-all duration-700 ${
          isUnity ? "h-8 backdrop-blur-md border-b" : "h-14 pt-2"
        } ${barBg}`}
      >
        <div className="flex items-center gap-1.5 h-full">
          <div
            className={`flex items-center gap-2 px-3 h-full cursor-pointer transition-colors rounded-lg ${hoverBg}`}
            onClick={toggleCalendarOverlay}
          >
            <span
              className={`text-sm font-bold transition-colors duration-500 ${
                isUnity && !hasMaximizedWindow
                  ? mutedTextColor
                  : "text-white drop-shadow-md"
              }`}
            >
          ...
          <Divider />
          <div
            className={`flex items-center px-3 h-full cursor-pointer transition-colors rounded-lg relative group ${hoverBg}`}
            onClick={toggleNotifications}
          >
            <Bell
              size={14}
              className={`transition-all duration-500 -rotate-[15deg] ${
                isUnity && !hasMaximizedWindow
                  ? mutedTextColor
                  : "text-white drop-shadow-md"
              }`}
        ...
        <div className="flex items-center gap-1 h-full">
          <div
            className={`flex items-center gap-4 px-4 h-full transition-all duration-500`}
          >
            {wifiEnabled && (
              <Wifi
                size={14}
          ...
          {activeMembers.length > 0 && (
            <>
              <Divider />
              <div
                className={`flex items-center -space-x-2 px-2 py-1 rounded-full cursor-pointer transition-all hover:scale-105 ${
                  membersPanelOpen
                    ? isDark
                      ? "bg-white/10"
                      : "bg-black/10"
                    ...
                    className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-[8px] font-bold ${
                      isUnity
                        ? isDark
                          ...
                          : "border-white bg-zinc-200 text-black"
                        : "border-black/20 bg-white/20 backdrop-blur-md text-white"
                    }`}
                  >
                    +{activeMembers.length - 3}
                  </div>
          ...
          {currentUser && (
            <>
              <Divider />
              <div
                className="flex items-center px-2 h-full cursor-pointer hover:opacity-80 transition-opacity"
                onClick={toggleAccountSwitcher}
              >
                <div
                  className={`w-6 h-6 rounded-full overflow-hidden border transition-all duration-500 ${
                    isUnity && !hasMaximizedWindow
                      ? "border-transparent"
          ...
          <Divider />
          <div
            className={`flex items-center px-4 h-full cursor-pointer transition-colors rounded-lg ${hoverBg}`}
            onClick={toggleControlCenter}
          >
