    ...
    <>
      <div
        className={`fixed bottom-0 left-0 right-0 h-2 z-[800] bg-transparent transition-opacity ${
          isDockHidden ? "opacity-100" : "opacity-0 pointer-events-none"
        }`}
        onMouseEnter={() => setIsHoveringEdge(true)}
      />
      {isUnity ? (
        <aside
          className={`fixed left-0 top-8 bottom-0 flex-col items-center py-6 backdrop-blur-3xl border-r z-[900] transition-all duration-500 rounded-r-[2rem] ${
            dockCollapsed ? "w-12" : "w-20"
          } ${sidebarBg} ${
            isDockHidden ? "-translate-x-full" : "translate-x-0"
          }`}
        >
          <div className="flex flex-col items-center gap-6 w-full">
            {LauncherBtn}
            <div className={`w-8 h-px ${dividerColor}`} />
          ...
          <div className="mt-auto flex flex-col items-center gap-4 w-full">
            ...
            <button
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${iconColor} ${iconBgHover}`}
            >
              <Terminal size={18} />
            </button>
            <button
        ...
        <div
          onMouseEnter={() => setIsHoveringEdge(true)}
          className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-[900] flex items-end gap-3 transition-all duration-700 cubic-bezier(0.32,0.72,0,1) ${
            isDockHidden
          ...
          >
            <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`w-[53px] h-10 rounded-full bg-cover bg-center transition-all ${
                    activeWorkspaceId === ws.id
                      ? "border-2 border-white brightness-110 shadow-lg"
              ...
              <button
                ...
                className="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/10 text-zinc-400 transition-all border border-white/10"
              >
                <Plus size={18} />
              </button>
            </div>
          ...
          >
            <div className="flex items-center gap-3">
              <div className="relative group/media">
                <div
                  className={`w-10 h-10 rounded-full overflow-hidden border-2 transition-transform duration-500 shadow-lg ${
                    isPlaying
