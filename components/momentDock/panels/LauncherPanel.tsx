...
import { DesktopLayout, AppId, OSMode } from "../../../types";

export const LauncherPanel: React.FC<{ anchorRef?: React.RefObject<HTMLDivElement> }> = ({ anchorRef }) => {
  const { launcherSearchQuery, setLauncherSearchQuery, toggleLauncher } =
    useDockStore();
  const { openWindow } = useWindowStore();
  const { theme } = useOSStore();
  const { layout, isMobile, isTablet, isTV, isGaming } = useSphereStore();
  ...
  }, [launcherSearchQuery, isMobile, isTablet, isTV, isGaming]);
  ...
  return (
    <div
      style={!isUnity && pos ? { left: pos.left, bottom: pos.bottom } : undefined}
      className={`
      flex flex-col backdrop-blur-3xl z-[3000] overflow-hidden border transition-all
      ${panelBg}
      ${
        isUnity
          ? "fixed left-[calc(80px+1rem)] top-4 bottom-4 w-[540px] rounded-[1.5rem] animate-in fade-in slide-in-from-left-4 duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]"
          : "fixed w-[580px] h-[70vh] max-h-[70vh] rounded-[2.5rem] animate-in fade-in slide-in-from-bottom-10 zoom-in-95 duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] origin-bottom-left"
      }
    `}
    >
      <div
                ...
                {filteredApps
                  ...
                  .map((app) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-[11px] transition-colors text-center leading-tight tracking-tight w-full truncate px-1 ${textColor}`}
                      >
                        {app.title}
                      </span>
                    </button>
      ...
      >
        ...
        <div className="relative w-full max-w-[460px] group">
          ...
          <input
            autoFocus
            className={`w-full py-4 pl-14 pr-8 rounded-[1.25rem] text-sm outline-none border transition-all ${inputBg} ${inputFocus}`}
