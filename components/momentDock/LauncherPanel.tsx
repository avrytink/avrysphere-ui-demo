...
export const LauncherPanel: React.FC = () => {
  const { launcherSearchQuery, setLauncherSearchQuery, toggleLauncher } = useDockStore();
  ...
  const panelBg = isDark ? 'bg-zinc-900/80 border-white/10 shadow-[0_30px_60px_rgba(0,0,0,0.5)]' : 'bg-white/85 border-white/40 shadow-[0_20px_50px_rgba(0,0,0,0.15)]';
  const inputBg = isDark ? 'bg-white/5 border-white/10 text-white placeholder:text-zinc-400' : 'bg-black/5 border-black/10 text-zinc-900 placeholder:text-zinc-500';
  const inputFocus = isDark ? 'focus:border-white/30 focus:bg-white/10' : 'focus:border-black/20 focus:bg-black/10';
  const textColor = isDark ? 'text-zinc-200 group-hover:text-white' : 'text-zinc-700 group-hover:text-black';
  ...
  return (
    <div className={`
      ...
      ${isUnity 
          ...
          : 'fixed bottom-[110px] left-1/2 -translate-x-1/2 w-[580px] h-[680px] max-h-[80vh] animate-in fade-in slide-in-from-bottom-8 zoom-in-95 duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] origin-bottom'
      }
    `}>
      <div 
      ...
      >
        ...
        >
          {Array.from({ length: totalPages }).map((_, pageIndex) => (
            <div key={pageIndex} className="w-full h-full flex-shrink-0 p-10 pt-12">
              <div className="grid grid-cols-4 gap-x-6 gap-y-10 place-items-start justify-items-center">
                {filteredApps.slice(pageIndex * ITEMS_PER_PAGE, (pageIndex + 1) * ITEMS_PER_PAGE).map(app => (
                  <button 
                    key={app.id}
                    onClick={() => { openWindow(app.id); toggleLauncher(); }}
                    className="flex flex-col items-center gap-3 cursor-pointer group w-24 focus:outline-none"
                  >
                    <div className={`w-[5.2rem] h-[5.2rem] rounded-full flex items-center justify-center shadow-xl group-hover:scale-110 transition-all duration-500 group-active:scale-95 group-hover:shadow-2xl border overflow-hidden ${iconBorder} ${app.iconImage ? 'bg-black' : `bg-gradient-to-br ${app.gradient}`}`}>
                      {app.iconImage ? (
                    ...
                    <span className={`text-[11px] font-bold transition-colors text-center leading-tight tracking-tight w-full truncate px-1 uppercase ${textColor}`}>
                      {app.title}
                    </span>
                  </button>
                ))}
      ...
      <div className={`flex flex-col items-center gap-6 p-8 pt-2 pb-12 shrink-0`}>
        {totalPages > 1 && (
            ...
            {Array.from({ length: totalPages }).map((_, i) => (
              <button
                ...
                className={`
                  ...
                  ${i === page 
                    ? (isDark ? 'bg-white w-2.5 h-2.5 scale-110' : 'bg-black w-2.5 h-2.5 scale-110') 
                    : (isDark ? 'bg-white/20 hover:bg-white/40 w-1.5 h-1.5' : 'bg-black/20 hover:bg-black/40 w-1.5 h-1.5')
                  }
                `}
              />
            ))}
        ...
        <div className="relative w-full max-w-[460px] group">
          <Search size={18} className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-blue-500 transition-colors" />
          <input 
            autoFocus
